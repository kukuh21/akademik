...
@section('content')
<form action="{{route('perkuliahan.update', ['id' => $data->id_perkuliahan])}}" enctype="multipart/form-data" method="POST" class="bg-white shadow-sm p-3">
@csrf
  ...
  <div class="box-body">
    <div class="row">
        ...
        <div class="form-group">
          ...
          <select name="id_mahasiswa" id="id_mahasiswa" class="form-control js-example-basic-single">
            ...
            @foreach ($mahasiswa as $list)
              <option value="{{ $list->id }}" {{ $data->id_mahasiswa == $list->id ? 'selected' : '' }}>
                {{ $list->Nim }} - {{ $list->Nama_Mhs}}
              </option>
            @endforeach
          </select>
        ...
        <div class="form-group">
          ...
          <select name="id_dosen" id="id_dosen" class="form-control js-example-basic-single">
            ...
            @foreach ($dosen as $list)
              <option value="{{ $list->id }}" {{ $data->id_dosen == $list->id ? 'selected' : '' }}>
                {{ $list->Nip }} - {{ $list->Nama_Dosen}}
              </option>
            @endforeach
          </select>
          <div class="invalid-feedback">
        ...
        <div class="form-group">
          ...
          <select name="id_matakuliah" id="id_matakuliah" class="form-control js-example-basic-single">
            ...
            @foreach ($matakuliah as $list)
              <option value="{{ $list->id }}"  {{ $data->id_matakuliah == $list->id ? 'selected' : '' }}>
                {{ $list->Kode_MK }} - {{ $list->Nama_MK}}
              </option>
            @endforeach
          </select>
          <div class="invalid-feedback">
      ...
      <div class="col-md-3">
        <div class="form-group">
          <label>Nilai</label>
          <input value="{{ $data->Nilai }}" type="text" class="form-control {{$errors->first('Nilai') ? "is-invalid": ""}}"  name="Nilai" id="Nilai">
          <div class="invalid-feedback">
              {{$errors->first('Nilai')}}
          </div>
        </div>
